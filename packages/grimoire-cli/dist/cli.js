...
const DIM = '#7F8C8D';
const RED = '#E74C3C';
const brand = (t) => chalk.hex(EMERALD)(t);
const accent = (t) => chalk.hex(GOLD)(t);
const muted = (t) => chalk.hex(DIM)(t);
const dark = (t) => chalk.hex(DARK_EMERALD)(t);
const error = (t) => chalk.hex(RED)(t);
function banner() {
    console.log('');
    console.log(dark('  ╔══════════════════════════════════════════════════════════════╗'));
...
async function saveRegistryIndex(entries) {
    await writeFile(REGISTRY_INDEX, JSON.stringify(entries, null, 2), 'utf-8');
}
async function findSpellFile(name, dir) {
    try {
        const files = await readdir(dir);
        for (const file of files) {
            if (!file.endsWith('.grimoire'))
                continue;
            const raw = await readFile(join(dir, file), 'utf-8');
            const spell = YAML.parse(raw);
            if (spell.name === name)
...
async function cmdSearch(query) {
    ...
    for (const entry of results) {
        ...
        if (s.tags)
            console.log(`    ${muted(s.tags.join(' · '))}`);
    }
}
async function cmdInstall(spellName) {
    banner();
    await ensureDirs();
    await syncRepoSpells();
    const index = await getRegistryIndex();
...
async function cmdCast(spellName, options) {
    banner();
    await ensureDirs();
    const index = await getRegistryIndex();
...
async function cmdPublish(filePath) {
    ...
    const newEntry = { spell, installed: false, path: dest };
    if (existing >= 0) {
